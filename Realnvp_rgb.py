...
class ResidualBlock(nn.Module):
    def __init__(self, dim, bottleneck, weight_norm):
        ...
        if bottleneck:
            ...
            self.res_block = nn.Sequential(
                WeightNormConv2d(dim, dim, (3, 3), stride=1, padding=1, 
                    bias=False, weight_norm=weight_norm, scale=True))
...
class RealNVP(nn.Module):
    def __init__(self, datainfo, prior, hps):
        ...
        if datainfo.name == 'cifar10':
            ...
            if datainfo.name == 'imnet32':
                ...
                # SCALE 4: 24 x 8 x 8
                self.s4_ckbd = self.checkerboard_combo(chan//3, dim, size, hps, final=True)
    ...
    def g(self, z):
        ...
        if self.datainfo.name in ['imnet32', 'imnet64', 'celeba']:
            ...
            if(self.datainfo.name == "imnet32" or self.datainfo.name == "celeba"):
                for i in reversed(range(len(self.s4_ckbd[0]))):
                    ...
                    x3, _ = self.s4_ckbd[2][i](x3, reverse=True)
        ...
        if(self.datainfo.name == "cifar10"):
            for i in range(len(self.s2_ckbd[0])-1,-1,-1):
                x1, _ = self.s2_ckbd[0][i](x1, reverse=True)
                x2, _ = self.s2_ckbd[1][i](x2, reverse=True)
                x3, _ = self.s2_ckbd[2][i](x3, reverse=True)

            x1 = self.restore(x1, x_off_1, self.order_matrix_1)
            x1 = self.squeeze(x1)

            x2 = self.restore(x2, x_off_1, self.order_matrix_1)
            x2 = self.squeeze(x2)

            x3 = self.restore(x3, x_off_1, self.order_matrix_1)
            x3 = self.squeeze(x3)
    ...
    def f(self, x):
        ...
        # We need to make changes to s2,s4 and s5 for splitting into 3 layers
        ...
        z1, z2, z3 = z, z, z
        log_diag_J1, log_diag_J2, log_diag_J3 = log_diag_J, log_diag_J, log_diag_J

        # SCALE 2: 16(32) x 16(32)
        if(self.datainfo.name == "cifar10"):
            for i in range(len(self.s2_ckbd[0])):
                z1, inc1 = self.s2_ckbd[0][i](z1)
                log_diag_J1 = log_diag_J1 + inc1

                z2, inc2 = self.s2_ckbd[1][i](z2)
                log_diag_J2 = log_diag_J2 + inc2

                z3, inc3 = self.s2_ckbd[2][i](z3)
                log_diag_J3 = log_diag_J3 + inc3
        else:
        ...
        if self.datainfo.name in ['imnet32', 'imnet64', 'celeba']:
            ...
            chunk_z = torch.chunk(z, 3, dim=1)  # 3 chunks
            chunk_log_det = torch.chunk(log_diag_J, 3, dim=1)
            # print(self.order_matrix_3.shape)
            # print(self.order_matrix_2.shape)
            # print(self.order_matrix_1.shape)

            z1, z2, z3 = chunk_z[0], chunk_z[1], chunk_z[2]
            log_diag_J1, log_diag_J2, log_diag_J3 = chunk_log_det[0], chunk_log_det[1], chunk_log_det[2]
            # print(z1.shape)
            # print(z_off_3.shape)
            # print(z_off_2.shape)
            # print(z_off_1.shape)
            ...
            if(self.datainfo.name == "imnet32" or self.datainfo.name == "celeba"):
                for i in range(len(self.s4_ckbd[0])):
                    z1, inc1 = self.s4_ckbd[0][i](z1)
                    log_diag_J1 = log_diag_J1 + inc1

                    z2, inc2 = self.s4_ckbd[1][i](z2)
                    log_diag_J2 = log_diag_J2 + inc2

                    z3, inc3 = self.s4_ckbd[2][i](z3)
                    log_diag_J3 = log_diag_J3 + inc3
            ...
            #     for i in range(len(self.s5_ckbd[0])):
            #         z1, inc1 = self.s5_ckbd[0][i](z1)
            #         log_diag_J1 = log_diag_J1 + inc1

            #         z2, inc2 = self.s5_ckbd[1][i](z2)
            #         log_diag_J2 = log_diag_J2 + inc2

            #         z3, inc3 = self.s5_ckbd[2][i](z3)
            #         log_diag_J3 = log_diag_J3 + inc3

            #     z1 = self.restore(z1, z_off_4, self.order_matrix_4)
            #     log_diag_J1 = self.restore(log_diag_J1, log_diag_J_off_4, self.order_matrix_4)

            #     z2 = self.restore(z2, z_off_4, self.order_matrix_4)
            #     log_diag_J2 = self.restore(log_diag_J2, log_diag_J_off_4, self.order_matrix_4)

            #     z3 = self.restore(z3, z_off_4, self.order_matrix_4)
            #     log_diag_J3 = self.restore(log_diag_J3, log_diag_J_off_4, self.order_matrix_4)

            z = torch.cat((z1, z2, z3), dim=1)
            log_diag_J = torch.cat((log_diag_J1, log_diag_J2, log_diag_J3), dim=1)
